...
* HTML에서 구현한 화면에서 이벤트가 발생하면 브라우저에 이벤트 감지를 전달하는 역할 - html : eventSource - JS    : eventHandler - html이 먼저 메모리에 load가 완료 되어야 JS가 기능을 적용일할 수 있다 - window\(소유주\).onload   function\( \){구현} = 기능
