...
* Servlet - req, res는 doGet과 doPost함수만이 주입받을 수 있는 객체이다. - 다른 함수를 선언할 수는 있지만 req, res객체를 주입 받을 수 없으므로 의미가 없다. - doGet, doPost함수만 정의해 사용하자 - 바로 자바로 연결된다. - Servlet을 실행하려면 브라우저가 필요하고, 브라우저는 URL이 필요하므로 web.xml배치서술자에 url을 등록해야한다. - 이때 등록된 Servelt class-name으로 인스턴스화를 할 수 있다.=자바를 제어할 수 있다. - 전송방식을 구분해주는 용도
