...
* **Controller** - 데이터와 비즈니스 로직 사이의 상호동작 관리 - 모델이 "어떻게"처리할지를 알려주는 역할 - 모바일에서는 화면의 로직처리도 담당한다. - 화면에서 사용자 요청을 받아 처리되는 부분을 구현, 요청을 분석해 모델, 뷰에 업데이트 요청 - 중개인 역할 - 모델의 상태를 변경하는 명령을 모델에 내릴 수 있다.\(워드프로세서에서 문서편집\) - 해당 뷰에 모델의 표시 방법을 변경하는 명령을 내릴 수 있다.\(문서를 스크롤하는 것\)
