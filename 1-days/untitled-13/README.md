...
* HTML안에 자바코드를 사용해 동적으로 웹페이지를 생성할 수 있게 해주는 서버 쪽 페이지 - JSP파일이 요청하면 서버가 요청을 받아 응답해야한다. - WAS제품이 java코드를 해석해 브라우저에 적용되도록 해준다. - java코드를 작성했으므로 컴파일\(서블릿 필요\)이 일어나고, print를 사용해 화면에 출력한다.
