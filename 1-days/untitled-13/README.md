...
1. JSP파일이 요청하면 서버가 요청을 받아 응답해야한다. - 브라우저가 url을 받아 응답해야하는데 java는 url이 없다. -&gt; servlet활용
2. WAS제품이 java코드를 해석해 브라우저에 적용되도록 해준다.
3. java코드를 작성했으므로 컴파일이 일어나고, print를 사용해 화면에 출력된다.
