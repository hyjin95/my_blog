...
* id는 중복값을 가지면 안되므로 배열로 생성되는 것 자체가 말이안되고, name은 중복될 수 있기 때문에 name을 key값으로 하는 Map에 사용자들의 정보를 담을 수 있기 때문이다.
* id는 jsp페이지 내에서만 활용되고, name은 컨트롤러로 가져가서 활용할 수 있기 때문이다. - &lt;form&gt;을 submit하여 액션을 타면 컨트롤러에서 요청객체에 name을 받아올 수 있다.
